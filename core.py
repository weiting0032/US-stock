...
# P8：回檔閘補強——舊式只有天花板（close ≤ SMA20×1.04），單日崩跌深破 SMA20 也算「回檔」，實為接刀
ENTRY_PULLBACK_FLOOR_ENABLE = get_env_int("ENTRY_PULLBACK_FLOOR_ENABLE", 1)  # 1=回檔須在 SMA20×(1−pct) 之上（跌太深=趨勢損壞，不是回檔）
ENTRY_PULLBACK_CONFIRM = get_env_int("ENTRY_PULLBACK_CONFIRM", 1)            # 1=回檔須有反轉確認（收漲或收復昨高）才進

# P11：SOX 板塊准入（軟閘）——SPY/QQQ regime 對「類股背離」（大盤多頭、半導體走熊）全盲；
# 半導體標的在 SOX BEAR 時新倉/加碼風險預算縮量（設 0=禁入）。出場邏輯不受影響。
SOX_GATE_ENABLE = get_env_int("SOX_GATE_ENABLE", 1)
SOX_GATE_BEAR_RISK_SCALE = get_env_float("SOX_GATE_BEAR_RISK_SCALE", 0.5)
...
def should_send_alert(alerts_df, ticker, action, current_price, current_score, current_session, target_price=None, state_changed=False) -> bool:
    # P12：出場警報攸關執行安全，繞過指紋/節流一律發送——崩盤日寧可重複也不可漏發
    # （掃描每日僅 2 次，重複量有界；買進/加碼類仍走正常去抖動）
    if str(action).upper() == "SELL_EXIT":
        return True
...
def pullback_entry_ok(close: float, sma20: float, prev_close: float, prev_high: float) -> bool:
    """P8：回檔買點＝「SMA20 ±帶狀區」＋「反轉確認」（三個掃描引擎共用）。

    舊式只有天花板（close ≤ SMA20×(1+pct)）：一檔還在 SMA200 上但單日崩 12%
    跌穿 SMA20 很深的股票照樣放行——那是接刀，不是回檔。補兩件事：
      地板（ENTRY_PULLBACK_FLOOR_ENABLE）：close ≥ SMA20×(1−pct)，跌太深視為趨勢損壞
      確認（ENTRY_PULLBACK_CONFIRM）：收漲（>昨收）或收復昨高，要求出現買盤跡象再進
    """
    if sma20 <= 0 or close > sma20 * (1 + ENTRY_PULLBACK_SMA20_PCT):
        return False
    if ENTRY_PULLBACK_FLOOR_ENABLE and close < sma20 * (1 - ENTRY_PULLBACK_SMA20_PCT):
        return False
    if ENTRY_PULLBACK_CONFIRM and not (close > prev_close or close >= prev_high):
        return False
    return True
...
def evaluate_strategy(
    ...
    sox_trend: Optional[str] = None,    # P11：SOX 板塊趨勢（BULL/NEUTRAL/BEAR；由呼叫端以因果方式取得）
    ...
    # P11：SOX 板塊軟閘——半導體標的在 SOX 空頭時風險縮量（可設 0=禁入）；非半導體不受影響
    _sox_scale = 1.0
    if SOX_GATE_ENABLE and sox_trend == "BEAR" and normalize_ticker(ticker) in US_SEMI_CATEGORY_MAP:
        _sox_scale = max(0.0, min(1.0, SOX_GATE_BEAR_RISK_SCALE))
    risk_dollars = (total_assets * limits["risk_per_trade_pct"]
                    * safe_float(market_regime.get("risk_multiplier", 1.0)) * _corr_scale * _sox_scale)
    ...
    _prev_row = hist.iloc[-2] if len(hist) >= 2 else last
    pullback_ok_entry = pullback_entry_ok(close, sma20,
                                          safe_float(_prev_row["Close"]), safe_float(_prev_row["High"]))  # P8
    ...
    return score, action, {
        ...
        "sox_gate_scale": round(_sox_scale, 2),     # P11：SOX 板塊軟閘縮量係數（1.0=未觸發）
...
def enrich_portfolio_with_weight_and_risk(portfolio: List[Dict], total_assets: float, cash: float, market_regime: Dict) -> List[Dict]:
    ...
    sox_trend = get_current_sox_trend()                                # P11：整組算一次（TTL 快取）
    for p in portfolio:
        ...
        if hist is not None and not hist.empty:
            sc, act, det, _ = evaluate_strategy(
                ...
                avg_corr=avg_corr, sox_trend=sox_trend,
            )
            row.update({
                "Signal": act,
                "SignalScore": sc,
...
def run_auto_scanner(portfolio, trades_df, cash, total_assets, market_regime, watchlist_df=None) -> Dict:
    ...
    for tk in universe:
        ...
        sc, act, det, note = evaluate_strategy(
            ...
            recent_buy=recent_buy, recent_sell=recent_sell, avg_corr=avg_corr, sox_trend=sox_trend,
        )
...
@ttl_cache(MARKET_CACHE_TTL, maxsize=4)
...
def get_current_sox_trend() -> Optional[str]:
    """P11：目前 SOX 板塊趨勢（BULL/NEUTRAL/BEAR），供 evaluate_strategy 軟閘。
    閘門停用或取價失敗回 None（＝不觸發縮量）。已含 TTL 快取，可放心逐掃描呼叫。"""
    if not SOX_GATE_ENABLE:
        return None
    try:
        return _get_sox_regime().get("trend")
    except Exception:
        return None
...
def _us_semi_score_one(ticker: str, sox_regime: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        pullback_ok = pullback_entry_ok(close, sma20,
                                        safe_float(prev["Close"]), safe_float(prev["High"]))  # P8
        not_extended = (atr <= 0) or (close <= sma20 + ENTRY_MAX_EXT_ATR * atr)
...
def apply_entry_risk_gates(cand: Dict, portfolio: List[Dict], total_assets: float,
                           ...
                           sox_trend: Optional[str] = None) -> None:
    """
    ...
    #    ＋ SOX 板塊軟閘（P11；本引擎候選皆為半導體，不需逐檔判別）
    _cs = corr_risk_scale(avg_corr)
    _ss = 1.0
    if SOX_GATE_ENABLE and sox_trend == "BEAR":
        _ss = max(0.0, min(1.0, SOX_GATE_BEAR_RISK_SCALE))
    qty = math.floor(total_assets * limits["risk_per_trade_pct"] * risk_mult * _cs * _ss / risk_per_share)
    if _cs < 1.0:
        cand["corr_risk_scale"] = round(_cs, 2)
    notes = []
    if _ss < 1.0:
        cand["sox_gate_scale"] = round(_ss, 2)
        notes.append(f"✂️ SOX 空頭：風險縮至 {_ss * 100:.0f}%" if _ss > 0 else "🚫 SOX 空頭：暫停半導體新倉")
...
def run_us_semi_scanner(extra_tickers: Optional[List[str]] = None,
    ...
    for r in results:
        ...
        apply_entry_risk_gates(r, portfolio, total_assets, cash, heat_pct, market_regime,
                               avg_corr=avg_corr, sox_trend=sox_regime.get("trend"))
